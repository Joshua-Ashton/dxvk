...
namespace dxvk {
  ...
  void STDMETHODCALLTYPE D3D10Device::CopySubresourceRegion(
    ...
    if (dstResourceDim == D3D10_RESOURCE_DIMENSION_BUFFER) {
      ...
      if (dstFormatInfo->elementSize != srcFormatInfo->elementSize) {
        Logger::err(str::format(
          "D3D10: CopySubresourceRegion: Incompatible texel size"
      ...
      if (dstImage->info().sampleCount != srcImage->info().sampleCount) {
        Logger::err(str::format(
          "D3D10: CopySubresourceRegion: Incompatible sample count",
      ...
      if (!util::isBlockAligned(srcOffset, srcFormatInfo->blockSize)
       || !util::isBlockAligned(dstOffset, dstFormatInfo->blockSize)) {
        Logger::err("D3D10: CopySubresourceRegion: Unaligned block offset");
        return;
      }
      ...
      if (!util::isBlockAligned(srcOffset, regExtent, srcFormatInfo->blockSize, srcExtent)
       || !util::isBlockAligned(dstOffset, regExtent, dstFormatInfo->blockSize, dstExtent)) {
        Logger::err("D3D10: CopySubresourceRegion: Unaligned block size");
        return;
      }
  ...
  bool D3D10Device::WaitForResource(
	  ...
	  // when a map fails with D3D10_MAP_FLAG_DO_NOT_WAIT set
	  if (!TestOption(D3D10Option::AllowMapFlagNoWait))
		  MapFlags &= ~D3D10_MAP_FLAG_DO_NOT_WAIT;

	  // Wait for the any pending D3D10 command to be executed
