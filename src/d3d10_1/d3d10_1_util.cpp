...
namespace dxvk {
  ...
  VkSamplerAddressMode DecodeAddressMode(
    ...
    switch (mode) {
      ...
      default:
        Logger::err(str::format("D3D10: Unsupported address mode: ", mode));
        return VK_SAMPLER_ADDRESS_MODE_REPEAT;
    }
  }
  ...
  VkBorderColor DecodeBorderColor(const FLOAT BorderColor[4]) {
    ...
    Logger::warn(str::format(
      "D3D10Device: No matching border color found for (",
  ...
  VkCompareOp DecodeCompareOp(D3D10_COMPARISON_FUNC Mode) {
    switch (Mode) {
      case D3D10_COMPARISON_NEVER:          return VK_COMPARE_OP_NEVER;
      case D3D10_COMPARISON_LESS:           return VK_COMPARE_OP_LESS;
      case D3D10_COMPARISON_EQUAL:          return VK_COMPARE_OP_EQUAL;
      case D3D10_COMPARISON_LESS_EQUAL:     return VK_COMPARE_OP_LESS_OR_EQUAL;
      case D3D10_COMPARISON_GREATER:        return VK_COMPARE_OP_GREATER;
      case D3D10_COMPARISON_NOT_EQUAL:      return VK_COMPARE_OP_NOT_EQUAL;
      case D3D10_COMPARISON_GREATER_EQUAL:  return VK_COMPARE_OP_GREATER_OR_EQUAL;
      case D3D10_COMPARISON_ALWAYS:         return VK_COMPARE_OP_ALWAYS;
    }
    
    if (Mode != 0)  // prevent log spamming when apps use ZeroMemory
      Logger::err(str::format("D3D10: Unsupported compare op: ", Mode));
    return VK_COMPARE_OP_NEVER;
  }
  
  
  VkMemoryPropertyFlags GetMemoryFlagsForUsage(D3D10_USAGE Usage) {
